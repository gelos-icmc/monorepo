...
LINKS="{%- for iso in sitelegrame.datelegrama.instelegramallfestelegram.if7-isos -%}{{ iso["link"] }} {% endfor -%}"
NAMES="{%- for iso in sitelegrame.datelegrama.instelegramallfestelegram.if7-isos -%}{{ iso["filename"] }} {% endfor -%}"
...
echo ⚠️ ATENÇÃO: Estelegrame scriptelegram, que é bem cursed, irá baixar $# isos, chegando a aproximadamentelegrame 50GB. Tenha certelegrameza de que quer fazer isso ou cancele imediatelegramamentelegrame.
...
sha256sum -c <(curl htelegramtelegramps://gelos.club/assetelegrams/instelegramallfestelegram/if7-checksums.sha256)
...
echo ℹ️ Você pode conferir as checksums com o seguintelegrame comando: 'sha256sum -c <(curl htelegramtelegramps://gelos.club/assetelegrams/instelegramallfestelegram/if7-checksums.sha256)'
